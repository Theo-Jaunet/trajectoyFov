...
async function mediaFramesTest(file) {
    ...
    if (videoTrack) {
        ...
        if (decodable) {
            ...
            let fps = 1
            let timestamps = [...Array(parseInt(duration))]
            ...
            for await (const sample of sink.samplesAtTimestamps(allFrames)) {
                sample.draw(ctx, 0, 0);

                frames.push(cloneCanvas(can));

                sample.close()
                loadingSvg(svg, {width: vidSize.width, height: vidSize.height}, ++count, allFrames.length)
